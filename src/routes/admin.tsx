...
// Workshop slots per day (fixed structure)
const WORKSHOP_SLOTS: Record<string, { id: string; time: string; label: string; sortOrder: number }[]> = {
  day1: [
    { id: 'day1-morning', time: '09:00', label: 'Morning Workshop', sortOrder: 10 },
    { id: 'day1-afternoon', time: '13:00', label: 'Afternoon Workshop', sortOrder: 30 },
  ],
  day2: [
    { id: 'day2-morning', time: '09:00', label: 'Morning Workshop', sortOrder: 10 },
  ],
}
              ...
              {enrollees.map((e) => (
                <li key={e.userId} className="flex items-center gap-3 p-2.5 rounded-lg bg-neutral-50 border border-neutral-100">
                  <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center shrink-0">
                    <span className="text-xs font-bold text-indigo-700">{e.name.charAt(0).toUpperCase()}</span>
                  </div>
...
function CreateSessionModal({ defaultSlotId, onClose, onCreated }: {
  ...
  const [form, setForm] = useState({ slotId: defaultSlotId, group: 'A', topic: '', location: '', maxParticipants: 30 })
  ...
  // All workshop slot options across days
  const allWorkshopSlots = Object.values(WORKSHOP_SLOTS).flat()
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select value={form.slotId} onChange={(e) => set('slotId', e.target.value)} className="w-full border border-neutral-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                {allWorkshopSlots.map((s) => <option key={s.id} value={s.id}>{s.time} — {s.label}</option>)}
              </select>
            </div>
            <div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select value={form.type} onChange={(e) => set('type', e.target.value)} className="w-full border border-neutral-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input type="text" value={form.time} onChange={(e) => set('time', e.target.value)} placeholder="12:00" autoFocus className="w-full border border-neutral-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div className="col-span-2">
              <label className="block text-xs font-medium text-neutral-600 mb-1">Title <span className="text-red-500">*</span></label>
              <input type="text" value={form.title} onChange={(e) => set('title', e.target.value)} placeholder="e.g. Lunch Break" className="w-full border border-neutral-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
          </div>
          <div>
            <label className="block text-xs font-medium text-neutral-600 mb-1">Note</label>
            <input type="text" value={form.note} onChange={(e) => set('note', e.target.value)} placeholder="e.g. All groups" className="w-full border border-neutral-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-neutral-600 mb-1">Location</label>
              <input type="text" value={form.location} onChange={(e) => set('location', e.target.value)} placeholder="e.g. Main Hall" className="w-full border border-neutral-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-neutral-600 mb-1">Sort order</label>
              <input type="number" value={form.sortOrder} onChange={(e) => set('sortOrder', Number(e.target.value))} className="w-full border border-neutral-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
          </div>
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={onClose} className="h-9 px-4 text-sm font-medium text-neutral-600 hover:text-neutral-900">Cancel</button>
...
function WorkshopCard({ s, onDelete, onViewEnrollees }: {
    ...
    <div className="bg-white border border-neutral-200 rounded-xl shadow-sm p-4 flex flex-col gap-3">
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex items-center gap-1 shrink-0">
          <Link to="/admin/sessions/$id" params={{ id: s.id }} className="p-1.5 rounded hover:bg-neutral-100 text-neutral-500 hover:text-neutral-800" title="Edit">
      ...
      <div className="mt-auto space-y-2">
        <div className="flex items-center justify-between text-xs text-neutral-500 border-t border-neutral-100 pt-2">
          ...
          <div className="w-20 h-1.5 bg-neutral-100 rounded-full overflow-hidden">
            <div className="h-full bg-indigo-500 rounded-full" style={{ width: `${Math.min(100, (s.enrollmentCount / s.maxParticipants) * 100)}%` }} />
          </div>
        </div>
      </div>
    </div>
...
function AdminLayout() {
  ...
  const [modalWorkshopSlot, setModalWorkshopSlot] = useState<string | null>(null)
  ...
  return (
    <div className="min-h-screen bg-neutral-50">
      ...
      {modalWorkshopSlot && (
        <CreateSessionModal
          defaultSlotId={modalWorkshopSlot}
          onClose={() => setModalWorkshopSlot(null)}
          onCreated={() => { setModalWorkshopSlot(null); invalidate() }}
        />
      )}
      {agendaModal && (
        <AgendaSlotModal
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="space-y-12">
          {DAYS.map((day) => {
            const workshopSlots = WORKSHOP_SLOTS[day.id] ?? []
                    ...
                    return (
                      <div key={slot.id}>
                        <div className="flex items-center gap-3 mb-3">
                          ...
                          <h3 className="text-sm font-semibold text-neutral-700">{slot.label}</h3>
                          <button
                            onClick={() => setModalWorkshopSlot(slot.id)}
                            className="flex items-center gap-1 h-6 px-2 text-xs font-medium bg-indigo-600 hover:bg-indigo-700 text-white rounded-full transition-colors"
                          >
                        ...
                        {slotSessions.length === 0 ? (
                          <p className="text-sm text-neutral-400 italic pl-16">No sessions yet.</p>
                        ) : (
